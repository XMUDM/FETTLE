...
from common.loss import BPRLoss, EmbLoss, ILADTLoss, CLALoss
...
class NCL(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.cla_weight = config['global_alignment_weight']
        self.prototype_num = config['prototype_num']
        self.iladt_weight = config['clcr_weight']
        self.clcr_gamma = config['clcr_gamma']
        self.ga_gamma = config['ga_gamma']
        self.iladt_loss = ILADTLoss(gamma=self.clcr_gamma, dim=self.latent_dim)
        self.cla_loss = CLALoss(K=self.prototype_num, D=self.latent_dim,
                                                         gamma=self.ga_gamma)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.t_feat is not None:
            ...
            # mf_t_loss = self.bpr_loss(user_all_embeddings[users], text_feats[pos_items], text_feats[neg_items])
        if self.v_feat is not None:
            image_feats = self.image_trs(self.v_feat)
            # mf_v_loss = self.bpr_loss(user_all_embeddings[users], image_feats[pos_items], image_feats[neg_items])


        assert self.t_feat is not None and self.v_feat is not None, "t_feat and v_feat must be not None"


        iladt_loss = self.iladt_loss(user_all_embeddings, item_all_embeddings,
                                   image_feats, text_feats, interaction[0], interaction[1])

        cla_loss = self.cla_loss(user_all_embeddings, item_all_embeddings, image_feats, text_feats, interaction[0],
                                                           interaction[1])


        return mf_loss + self.reg_weight * reg_loss + self.iladt_weight * iladt_loss + self.cla_weight*cla_loss, ssl_loss, proto_loss
